...
int main()
{
    while(scanf("%d %d %d", &n, &m, &s) && n > 0)
    {
        ...
        //cout << "Orientation = " << robotOrientation << "(X,Y) = (" << robotPosition_x << "," << robotPosition_y << ")" << endl;
        scanf("%s", robotInstructions);
        for(int i = 0; robotInstructions[i] != '\0'; ++i)
        {
            //cout << "INST = " << robotInstructions[i] << endl;
            if(robotInstructions[i] == 'F')
            {
                moveRobot(robotOrientation, robotPosition_x, robotPosition_y);
                //cout << "(X,Y) = (" << robotPosition_x << "," << robotPosition_y << ")" << endl;
            ...
            //cout << "(X,Y) = (" << robotPosition_x << "," << robotPosition_y << ")" << "  Orientation = " << robotOrientation << endl;
        }
        printf("%d\n", numOfStickers);
    }
    return 0;
